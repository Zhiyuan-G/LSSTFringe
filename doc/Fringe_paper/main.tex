...
%which combines LSST baseline curve of filter, optics, mirrors, atmosphere and detector response together~\citep{Ivezi19}, 
...
%One potential area that fringing might have impact on is the measurement of low surface brightness objects (LSB), such as ultra diffuse galaxy.To better quantify the impact of fringing, we convert the unit of simulated images (Figure~\ref{fig:lsst-image}) from number of sky background photon to surface brightness ($mag/arcsec^{2}$), which is commonly used to characterize galaxy luminosity, based on the equation\footnote{\url{https://smtn-016.lsst.io}}:
...
%The LSST DM pipeline will implement filters that are similar to the Gaussian filter used in the previous section during image reduction process. 
...
%The unit of $\mu Jy$ is related to photon number by the relation:
%\begin{equation}
